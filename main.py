...
from model_stats import count_parameters, calculate_flops, print_model_stats 
from visualization import visualize_features  

def get_dataset_info(dataset_name):
    """获取数据集相关信息"""
    dataset_info = {
        'Houston': {  # Houston数据集信息
            'num_classes': 15,  # 类别数
            'HSI_Channel': 144,  # 高光谱图像通道
            'LiDAR_Channel': 1,  # LiDAR数据通道数
...
def initialize_model(args, dataset_info):
    ...
    # 使用DataParallel而不是DistributedDataParallel
    if args.multi_gpu and torch.cuda.device_count() > 1:
    ...
    # 计算模型参数量和FLOPs
    input_shape1 = (args.batch_size, dataset_info['HSI_Channel'], args.window_size, args.window_size)
    input_shape2 = (args.batch_size, dataset_info['LiDAR_Channel'], args.window_size, args.window_size)
    total_params, params_by_layer = count_parameters(model)
    total_flops, flops_by_layer = calculate_flops(model, input_shape1, input_shape2)
    ...
    print_model_stats(
        ...
        batch_size=args.batch_size,
        window_size=args.window_size,
        hsi_channels=dataset_info['HSI_Channel'],
        lidar_channels=dataset_info['LiDAR_Channel']
    )
    ...
    return model, optimizer, criterion, scheduler, device, model_stats
...
def main(args):
    ...
    model, optimizer, criterion, scheduler, device, model_stats = initialize_model(args, dataset_info)
