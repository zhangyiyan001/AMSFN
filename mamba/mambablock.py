...
class SS2D(nn.Module):
    ...
    def forward_corev1(self, x: torch.Tensor, float32=True):
        # float32 should be true in training!!!! otherwise, the output of selective_scan_mu would be inf...
        selective_scan = selective_scan_fn_v1
...
class CrossMambaFusion_SS2D_SSM(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            # to b, d, h, w
            y_rgb = rearrange(y_rgb, "b (h w) d -> b h w d", h=H, w=W)
            y_e = rearrange(y_e, "b (h w) d -> b h w d", h=H, w=W)
...
class CrossConcatMambaFusion(nn.Module):
    def __init__(self,
         dim: int = 0,
         drop_path: float = 0,
         norm_layer: Callable[..., torch.nn.Module] = partial(nn.LayerNorm, eps=1e-6),
         ...
         **kwargs):
        super().__init__()
        self.op1 = CrossMambaFusion_SS2D_SSM(
            d_model=dim,
            dropout=attn_drop_rate,
            d_state=d_state,
            ssm_ratio=ssm_ratio,
    ...
    def forward(self, x_hsi, x_lidar):
        hsi_cross, lidar_cross = self.op1(x_hsi, x_lidar)
        # hsi_cross = self.norm1(hsi_cross)
        # lidar_cross = self.norm1(lidar_cross)
        # hsi_cross = hsi_cross + self.norm1(self.drop_path1(hsi_cross))
        # lidar_cross = lidar_cross + self.norm2(self.drop_path2(lidar_cross))
        x_fuse = x_hsi + x_lidar + self.op2(hsi_cross, lidar_cross)
        # x_fuse = x_fuse + self.mlp(self.norm4(x_fuse))
...
class Backbone_VSSM(VSSM):
    def __init__(self, patch_size=1, in_chans=144,
                 depths=[2, 2, 9], dims=[96, 192, 384],
...
class SpectralMamba(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.mamba = Mamba(MambaConfig(d_model=d_model, n_layers=2))
...
class Conv1d(nn.Module):
    def __init__(self, in_cha, out_chan, ker_s, stride, padding):
        super().__init__()
        self.layer1 = nn.Sequential(nn.Conv1d(in_cha, out_chan, ker_s, stride, padding),
...
class SpectralModule(nn.Module):
    def __init__(self, in_chan):
        super().__init__()
        self.spectralmamba0 = SpectralMamba(121)
        self.conv1d_1 = Conv1d(in_chan,  92, 5, 2, 0)
        self.spectralmamba1 = SpectralMamba(59)
        self.conv1d_2 = Conv1d(92, 64, 5, 2, 0)
        self.spectralmamba2 = SpectralMamba(28)
        self.conv1d_3 = Conv1d(64, 32, 5, 2, 0)
        self.spectralmamba3 = SpectralMamba(12)

    def forward(self, x):
        x0 = self.spectralmamba0(x)
        x1 = self.conv1d_1(x0)
        x1 = self.spectralmamba1(x1)
        x2 = self.conv1d_2(x1)
        x2 = self.spectralmamba2(x2)
        x3 = self.conv1d_3(x2)
        x3 = self.spectralmamba3(x3)
        return x3
