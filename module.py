...
def generate_image_cubes(input_data, pixels_select, windowSize):
...
def GetImageCubes_all(input_data, pixels_select, windowSize=11):
    height = input_data.shape[0]
    width = input_data.shape[1]
    Band = input_data.shape[2]
    kind = np.unique(pixels_select).shape[0]   

    paddingdata = np.pad(input_data, ((30, 30), (30, 30), (0, 0)), "constant")  
    paddinglabel = np.pad(pixels_select, ((30, 30), (30, 30)), "constant")  
  
    y0 = np.ones((height, width))
    paddingx = np.pad(y0, ((30, 30), (30, 30)), "constant") 
